...
- The brand list is a **hardcoded `CASES` array in `script.js`** (inside the carousel IIFE, clearly marked ">>> MAINTAIN THE BRAND LIST HERE <<<"). Each row is `["Nome azienda", "Macro-area", "URL"]`. The 3rd value (URL) is optional: if present the box becomes a clickable `<a>` (opens in new tab, shown with a ↗ and `.carousel__item--link` styling); if `""` it's a plain `<div>`. Edit/add/remove rows there. Company names show as-is; **area labels auto-translate** to English via the shared translation cache (see [[i18n-architecture]]).
